"""prob2(블록 강결합)를 '비싼 목적함수' 영역(10분 @20s/eval = 30 evals)에서 재시도."""
import sys, numpy as np
sys.path.insert(0, ".")
from expensive_common import benchmark, CAP, random_search
...
_, J_star = ref.coordinate_ascent(rng, restarts=60)
rng = np.random.default_rng(7)
BASE_VAR = float(np.var([ref.true_objective(ref.random_solution(rng)) for _ in range(4000)]))
print(f"prob2 J*={J_star:.2f}, Var_random={BASE_VAR:.1f}")
...
benchmark(lambda s: Problem(seed=s), ALGOS, nmse, list(range(10)),
          "prob2 block — expensive objective", "prob2/expensive.png",
