...
### 4C. 어떤 효과까지 screening 할 것인가
- **필수: 인코딩된 모든 요인의 주효과(범주형은 수준 대비 포함).**
- **선택: 2-요인 상호작용은 "전부"가 아니라 "표적"만** — (a) 도메인상 큰 상호작용이 의심되는 쌍, (b) effect heredity로 주효과 큰 요인쌍.
  - 이유: 전체 2차는 C(k,2)로 폭발 → 예산 낭비. 강한 희소가정으로 후보 한정.
- **차수 상한**: screening 단계는 **주효과 + 표적 2차**까지. 3차↑는 screening 범위 밖(이후 모델/EA가 암묵 포착).
- 목표 해상도: 예산이 되면 **주효과를 2차와 분리(Res IV 유사)**, 빠듯하면 주효과 우선(Res III 유사) + 후속 증강.
