...
### Cardinality 프로파일 (생성기 knob, 실제 값으로 교체 가능)
- **Categorical(10):** `{30, 10, 6, 5, 4, 3, 3, 2, 2, 2}`
- **Ordinal(20):** `{8, 5, 4, 4, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2}`
- Σlog₁₀(level) ≈ 14.7 → 조합 ≈ 5.5×10¹⁴
- categorical은 고cardinality 허용(최대 30), ordinal은 중간 이하(2~8)
- **BM1/2/3는 동일한 X 구조/크기를 공유** → 난이도는 교호작용·다봉성·목적충돌·노이즈로만 조절(알고리즘 비교 공정성 확보)
